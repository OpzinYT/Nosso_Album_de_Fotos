      ...
      <div class="photo-side">
        ...
        <div class="photo-placeholder" id="ph-${i}">
          <svg viewBox="0 0 60 60" fill="none" xmlns="http://www.w3.org/2000/svg">
            <rect x="4" y="10" width="52" height="40" rx="3" stroke="white" stroke-width="1.5"/>
            <circle cx="22" cy="26" r="6" stroke="white" stroke-width="1.5"/>
            <path d="M4 40 L18 28 L30 38 L40 30 L56 44" stroke="white" stroke-width="1.5" stroke-linecap="round"/>
          </svg>
          <input type="file" accept="image/*" class="upload-overlay" onchange="loadImg(event,${i})" title="Clique para adicionar foto">
        </div>
        <div class="upload-hint">clique para adicionar foto</div>
...
function loadImg(event, idx) {
  const file = event.target.files[0];
  if (!file) return;
  const reader = new FileReader();
  reader.onload = e => {
    const ph = document.getElementById(`ph-${idx}`);
    ph.innerHTML = `<img src="${e.target.result}" alt="foto">`;
  };
